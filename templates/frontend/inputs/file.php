...
* File Input Template
* 
* This template can be overridden by copying it to yourtheme/wpcomment/frontend/inputs/file.php
* 
* @version 1.0
**/
...
$fm = new WPComment_InputManager($field_meta, 'file');
...
$btn_label   = ($btn_label == '' ? __('Select files', "wpcomment") : $btn_label);
?>


<div id="wpcomment-file-container-<?php echo esc_attr($fm->data_name()); ?>" class="<?php echo esc_attr($fm->field_inner_wrapper_classes()); ?>" >

	<!-- if title of field exist -->
	<?php if ($field_label): ?>
		<label class="<?php echo esc_attr($fm->label_classes()); ?>" for="<?php echo esc_attr($fm->data_name()); ?>" ><?php echo sprintf(__("%s", "wpcomment"), $field_label); ?></label>
	<?php endif ?>

	
	<div class="wpcomment-file-container text-center" style="height: auto;">
		<a 
			href="javascript:;" 
		...
		<span class="wpcomment-dragdrop-text"><?php echo _e("Drag File Here", "wpcomment"); ?></span>
	</div> <!-- wpcomment-file-container -->
	...
	<div id="filelist-<?php echo esc_attr($fm->data_name()); ?>" class="filelist <?php echo esc_attr($fm->data_name()); ?>">
		...
		if ( !empty( $default_value ) ){ 
			...
			foreach ($default_value as $key => $file) {
				...
				$file_preview = wpcomment_uploaded_file_preview($file['org'], $field_meta);
	        	if( !isset($file['org']) || $file_preview == '') continue;

	        	$file_name = $file['org'];
	        	$file_form_name = 'wpcomment[fields]['.$fm->data_name().']['.$key.'][org]';
	        	$file_form_class = 'wpcomment-file-cb wpcomment-file-cb-'.$fm->data_name();
	        	?>
