...
* 
* This template can be overridden by copying it to yourtheme/wpcomment/frontend/inputs/timezone.php
* 
* @version 1.0
**/
...
$fm = new WPComment_InputManager($field_meta, 'timezone');
...
$options = wpcomment_array_get_timezone_list($regions, $show_time);

if( !empty($first_option) ) {
	$options[''] = sprintf( __("%s","wpcomment"), $first_option );
}

if ( ! $options ) {
	
	echo '<div class="wpcomment-option-notice">';
        echo '<p>'. __( "The timezone not found, please add different regions.", "wpcomment" ) .'</p>';
    echo '</div>';
